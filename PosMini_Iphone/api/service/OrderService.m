...
-(void)requestForOrderRecord:(NSDictionary *)param{
    ...
    NSString* url = [NSString stringWithFormat:@"/mtp/action/query/mini/v2/queryPayOrders"];
    ...
    [dict setValue:[param objectForKey:PARAM_ORDER_BUSINESS_TYPE] forKey:PARAM_ORDER_BUSINESS_TYPE];
...
-(void)requestForOrderRecordStatistics:(NSDictionary *)param{
    ...
    NSString* url = [NSString stringWithFormat:@"/mtp/action/query/mini/statisticsSuccPayOrders"];
    ...
    PosMiniCPRequest *posReq = [PosMiniCPRequest postRequestWithPath:url andBody:dict];
    [posReq onRespondTarget:self selector:@selector(orderStatisticsRequestDidFinished:)];
    [posReq execute];

}

-(void)orderStatisticsRequestDidFinished:(PosMiniCPRequest *)req{
    [[PosMini sharedInstance] hideUIPromptMessage:YES];
